...
plotFileSats <- function(source_file, one_gnss=FALSE, invert=FALSE) {
  ...
  # get colour codes from integers
...
assignColourToSats <- function(intvect) {
  # get a bunch of unique colours
  qual_col_pals = brewer.pal.info[brewer.pal.info$category == 'qual',]
  col_vector = unlist(mapply(brewer.pal, qual_col_pals$maxcolors, rownames(qual_col_pals)))
  
  if (max(intvect, na.rm = TRUE) > length(col_vector)) {
    warning("Some satellite colours may be duplicated")
  }
  
  # give each sat a colour - wrap satnums around if they exceed the colours list
  return(col_vector[intvect%%length(col_vector)])
}
...
animateSats <- function(source_file) {
  ...
  ll <- read.table(paste0('ll_data/', source_file), sep=',', header = TRUE)
  sv <- read.table(paste0('sv_data/', source_file), sep=',', header = TRUE)
  as <- read.table(paste0('as_data/', source_file), sep=',', header = TRUE)
...
loadSatData <- function(file_name) {
  ...
  ll <- read.table(paste0('ll_data/', file_name), sep=',', header = TRUE)
  sv <- read.table(paste0('sv_data/', file_name), sep=',', header = TRUE)
